...
void usage(UsageEnvironment& env, char const* progName) {
  env << "Usage: " << progName << " <rtsp-url-1>  saveaddress.h264\n";
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  static FILE *fp = fopen(save_name.c_str(), "wb");
  fwrite(pH264, 1, totalSize, fp);
  fflush(fp);
  printf("\ttotalSize %d bytes  frameSize:%d   num:%d\n", totalSize,frameSize,num);
  free(pH264);
  }/*if 0 == strncmp(fSubsession.codecName(), "H264", 16)*/
  // Then continue, to request the next frame of data:
  continuePlaying();
